...
export default function SectionStrengths() {
  ...
  return (
    <section id="strengths" ref={secRef} className="relative" style={{ contain:"paint" }}>
      ...
      <div className="relative z-10 mx-auto max-w-6xl px-6 pt-4 pb-28 md:pt-6 md:pb-32">
        {/* Header */}
        <div className="flex flex-col items-center text-center"
          ...
          <span className="inline-flex items-center gap-2 rounded-full border border-white/18 bg-white/[0.07] px-4 py-2 text-xs tracking-widest text-white/85 backdrop-blur-sm">
            <span className="h-2 w-2 rounded-full bg-emerald-400" />OUR STRENGTHS
          </span>
          ...
          <p className="mt-4 max-w-2xl text-sm leading-relaxed text-white md:text-base">
        ...
        <div className="mt-10 md:mt-14 relative">
          ...
          <div className="md:hidden">
            <div className="flex justify-center mb-5">
              {mobileView==="trust"
                ? <span className="st-mCue inline-flex items-center gap-2 rounded-full px-4 py-2 text-xs text-white/80"><span className="h-2 w-2 rounded-full bg-emerald-400"/>Trust By — สิ่งที่เรายึดมั่น</span>
                : <span className="st-mCue inline-flex items-center gap-2 rounded-full px-4 py-2 text-xs text-white/80"><span className="h-2 w-2 rounded-full bg-sky-400"/>Provide To — สิ่งที่เราส่งมอบ</span>}
            </div>
            {mobileView==="trust"
            ...
            <div className="relative mx-auto max-w-6xl" key={`f${vk}`}>
              <div className="grid md:grid-cols-[1fr_280px_1fr] gap-8 mb-6">
                <div className="flex justify-center">
                  <span className="st-bdg inline-flex items-center gap-2 rounded-full border border-emerald-400/30 bg-emerald-500/10 px-5 py-2 text-sm font-medium text-emerald-200">
                    <span className="h-2 w-2 rounded-full bg-emerald-400 animate-pulse"/>Trust By — สิ่งที่เรายึดมั่น
                  </span>
                </div>
                <div />
                <div className="flex justify-center">
                  <span className="st-bdg inline-flex items-center gap-2 rounded-full border border-sky-400/30 bg-sky-500/10 px-5 py-2 text-sm font-medium text-sky-200">
              ...
              <div className="grid md:grid-cols-[1fr_280px_1fr] items-stretch gap-8">
                ...
                <div className="flex items-center justify-center" style={{minHeight:340}}>
                  <img src={cubeImg} alt="Cube" className={`w-[240px] select-none ${inV?"st-cube-anim":""}`} draggable={false} style={{filter:"drop-shadow(0 8px 24px rgba(56,224,208,.2))"}}/>
                </div>
          ...
          <div className={`strength-stage hidden md:block ${view==="trust"?"is-show":"is-hide"}`}>
            ...
            <div className="relative mx-auto grid max-w-5xl items-center gap-10 md:grid-cols-2" key={`t${vk}`}>
              ...
              <div className="relative flex flex-col items-center gap-8">
                <img src={cubeImg} alt="Cube" className={`w-[300px] select-none ${inV?"st-cube-anim":""}`} draggable={false} style={{filter:"drop-shadow(0 8px 24px rgba(56,224,208,.2))"}}/>
