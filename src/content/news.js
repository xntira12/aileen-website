import egatHero from "../assets/img/News/egat-safety-together/IMG_9815.jpeg";
import egatGalleryBooth from "../assets/img/News/egat-safety-together/20251015_082421.jpg";
import egatGalleryTeam from "../assets/img/News/egat-safety-together/20251015_082456.jpg";
import aiDrivenHero from "../assets/img/News/ai-driven/20250624_055131226_iOS.jpg";
import aiDrivenGalleryBooth from "../assets/img/News/ai-driven/20250624_033758040_iOS.jpg";
import sistamHero from "../assets/img/News/sistam/20250925_164810(2).jpg";
import sistamGalleryBooth from "../assets/img/News/sistam/20250925_150840.jpg";
import sistamGalleryVisitor from "../assets/img/News/sistam/20250926_141544.jpg";
import sistamGalleryBackdrop from "../assets/img/News/sistam/20250925_163913.jpg";
import tncheHero from "../assets/img/News/TNChE/720563591_122224379744509617_8918146535867206756_n (1).jpg";
import tncheGalleryBooth from "../assets/img/News/TNChE/719161022_122224379762509617_4484651737298028412_n.jpg";
import tncheGalleryDemo from "../assets/img/News/TNChE/720065152_122224379750509617_4797097085255949318_n.jpg";
import epvHero from "../assets/img/EPV/20260709_142338.jpg";
import epvAlbum from "../assets/img/EPV/LINE_ALBUM_Event aileen nintex 2026_260710_21.jpg";
import epvGallery2 from "../assets/img/EPV/20260709_055337766_iOS.jpg";
import epvGallery3 from "../assets/img/EPV/20260709_050828107_iOS.jpg";
import epvGallery4 from "../assets/img/EPV/20260709_035630385_iOS.jpg";
import gavalonNpcHero from "../assets/img/gavalon/GAVALON-2 (1).png";
import gavalonEventHero from "../assets/img/gavalon/gavalon-event2.png";

export const NEWS_ARTICLES = [
  {
    slug: "workshop-gavalon-legal-management-system-2026",
    title: "Workshop GAVALON: ระบบบริหารจัดการกฎหมายสำหรับองค์กร",
    category: "Event Highlight",
    date: "19 สิงหาคม 2569",
    publishedAt: "2026-08-05",
    location: "ออนไลน์ผ่าน Microsoft Teams",
    summary:
      "รู้ทันกฎหมายใหม่ คัดกรองเฉพาะกฎหมายที่เกี่ยวข้องกับธุรกิจ และบริหารความสอดคล้องขององค์กรได้อย่างเป็นระบบด้วย Gavalon — พร้อม Workshop ออนไลน์ ทดลองใช้งานระบบจริงและสอบถามทีมผู้เชี่ยวชาญ",
    excerpt:
      "Gavalon ช่วยให้องค์กรติดตาม จัดการ และประเมินกฎหมายที่เกี่ยวข้องกับธุรกิจได้ง่ายขึ้นในระบบเดียว ลดภาระการค้นหาข้อมูลจากหลายแหล่ง พร้อมทดลองใช้งานฟรี 30 วัน",
    tags: [
      "GAVALON",
      "Workshop",
      "Legal Compliance",
      "Legal Management",
      "Microsoft Teams",
      "I-Law",
    ],
    highlights: [
      "Workshop ออนไลน์ วันพุธที่ 19 สิงหาคม 2569 เวลา 13.30 – 14.30 น.",
      "ทดลองใช้งาน Gavalon 30 นาที พร้อมถาม–ตอบกับทีมผู้เชี่ยวชาญ",
      "ลูกค้า I-Law เดิมอัปเกรดใช้ Gavalon ฟรีด้วย User และ Password เดิม",
    ],
    sections: [
      {
        heading: "GAVALON — ระบบบริหารจัดการกฎหมายสำหรับองค์กร",
        paragraphs: [
          "รู้ทันกฎหมายใหม่ คัดกรองเฉพาะกฎหมายที่เกี่ยวข้องกับธุรกิจ และบริหารความสอดคล้องขององค์กรได้อย่างเป็นระบบ",
          "Gavalon ช่วยให้องค์กรสามารถติดตาม จัดการ และประเมินกฎหมายที่เกี่ยวข้องกับธุรกิจได้ง่ายขึ้นในระบบเดียว ลดภาระการค้นหาข้อมูลจากหลายแหล่ง พร้อมช่วยให้องค์กรมั่นใจว่ากฎหมายที่นำไปใช้อ้างอิงมีความเกี่ยวข้องและเหมาะสมกับธุรกิจของคุณ",
        ],
      },
      {
        heading: "ทำไมองค์กรจึงควรใช้ Gavalon",
        paragraphs: [
          "กฎหมายและข้อกำหนดที่เกี่ยวข้องกับการดำเนินธุรกิจมีการเปลี่ยนแปลงอยู่เสมอ การติดตามข้อมูลด้วยวิธีเดิมอาจทำให้องค์กรพลาดกฎหมายสำคัญ หรือเสียเวลาในการตรวจสอบข้อมูลจำนวนมากที่ไม่เกี่ยวข้องกับธุรกิจ",
          "Gavalon ถูกออกแบบมาเพื่อช่วยให้องค์กรสามารถบริหารจัดการข้อมูลด้านกฎหมายได้สะดวกและเป็นระบบ ตั้งแต่การติดตามกฎหมายใหม่ การคัดกรองกฎหมายที่เกี่ยวข้อง ไปจนถึงการบันทึกและตรวจสอบความสอดคล้องขององค์กร — แม่นยำ • ทันสมัย • เชื่อถือได้",
        ],
      },
      {
        heading: "ฟังก์ชันการใช้งานหลักของ Gavalon",
        paragraphs: [
          "01 ติดตามกฎหมายใหม่ที่เกี่ยวข้องทุกสัปดาห์ — รับข้อมูลกฎหมายใหม่ที่อาจเกี่ยวข้องกับธุรกิจขององค์กรอย่างสม่ำเสมอ ช่วยลดภาระในการค้นหาและติดตามกฎหมายด้วยตนเอง",
          "02 แสดงข้อมูลสำคัญของกฎหมายอย่างครบถ้วน — รวบรวมรายละเอียดสำคัญของกฎหมายไว้ในที่เดียว ช่วยให้ผู้ใช้งานสามารถตรวจสอบและทำความเข้าใจกฎหมายที่เกี่ยวข้องได้สะดวกมากขึ้น",
          "03 คัดลอกข้อความกฎหมายได้ทันที — สามารถนำข้อความหรือสาระสำคัญจากกฎหมายไปใช้อ้างอิง ประกอบการประเมิน หรือจัดทำเอกสารภายในองค์กรได้สะดวกยิ่งขึ้น",
          "04 ค้นหาและดาวน์โหลดไฟล์กฎหมายที่เกี่ยวข้อง — ค้นหาเอกสารและดาวน์โหลดไฟล์กฎหมายที่ต้องการได้จากระบบ ลดเวลาในการค้นหาเอกสารจากหลายแหล่งข้อมูล",
          "05 บันทึกความสอดคล้องด้านกฎหมายขององค์กร — ช่วยให้องค์กรสามารถบันทึกและติดตามสถานะการปฏิบัติตามกฎหมายที่เกี่ยวข้อง เพื่อรองรับการตรวจสอบและการทบทวนย้อนหลัง",
        ],
      },
      {
        heading: "มากกว่าระบบติดตามกฎหมาย",
        paragraphs: [
          "Gavalon ไม่ได้ช่วยเพียงแค่รวบรวมข้อมูลกฎหมาย แต่ยังช่วยให้องค์กรสามารถนำข้อมูลไปใช้งานต่อได้จริง",
          "คัดกรองกฎหมายที่เกี่ยวข้องกับธุรกิจ — ช่วยลดปริมาณข้อมูลที่ผู้ใช้งานต้องตรวจสอบ โดยมุ่งเน้นเฉพาะกฎหมายที่เกี่ยวข้องกับลักษณะธุรกิจและการดำเนินงานขององค์กร",
          "สอบถามและปรึกษาผู้เชี่ยวชาญ — เมื่อต้องการความชัดเจนเพิ่มเติม สามารถสอบถามหรือปรึกษาผู้เชี่ยวชาญด้านกฎหมายได้โดยตรง เพื่อประกอบการพิจารณาและตัดสินใจขององค์กร",
          "จัดเก็บประวัติและงานด้านกฎหมาย — เก็บข้อมูลและกิจกรรมที่เกี่ยวข้องกับกฎหมายไว้ในระบบเดียว สามารถตรวจสอบย้อนหลังได้เมื่อต้องการ",
          "ระบบใช้งานง่าย — ออกแบบให้ผู้ใช้งานสามารถค้นหา ตรวจสอบ และบริหารข้อมูลกฎหมายได้สะดวก ลดขั้นตอนการทำงานที่ซับซ้อน",
        ],
      },
      {
        heading: "ประโยชน์ที่องค์กรของคุณจะได้รับ",
        paragraphs: [
          "รู้ทันการเปลี่ยนแปลงของกฎหมาย — ติดตามกฎหมายใหม่อย่างสม่ำเสมอ ลดความเสี่ยงจากการพลาดข้อมูลสำคัญที่อาจส่งผลต่อธุรกิจ",
          "ลดเวลาในการค้นหาและคัดกรองกฎหมาย — เข้าถึงข้อมูลกฎหมายที่เกี่ยวข้องได้ง่ายขึ้น โดยไม่ต้องค้นหาข้อมูลจากหลายแหล่งด้วยตนเอง",
          "จัดการข้อมูลกฎหมายอย่างเป็นระบบ — รวบรวมเอกสาร ประวัติ และข้อมูลด้านความสอดคล้องไว้ในระบบเดียว ช่วยให้สามารถตรวจสอบย้อนหลังได้ง่าย",
          "เพิ่มความมั่นใจในการบริหาร Compliance — ช่วยให้องค์กรสามารถติดตามและบริหารการปฏิบัติตามกฎหมายได้อย่างต่อเนื่องและมีประสิทธิภาพ",
          "เข้าถึงผู้เชี่ยวชาญเมื่อต้องการคำแนะนำ — รองรับการสอบถามและปรึกษาประเด็นด้านกฎหมายเพิ่มเติมกับผู้เชี่ยวชาญ",
        ],
      },
      {
        heading: "เริ่มต้นใช้งาน Gavalon ได้ง่าย เพียง 3 ขั้นตอน",
        paragraphs: [
          "1. ลงทะเบียนทดลองใช้งาน — สแกน QR Code หรือเข้าสู่เว็บไซต์ Gavalon เพื่อเริ่มต้นลงทะเบียน",
          "2. กรอกข้อมูลองค์กร — สมัครทดลองใช้งานและระบุข้อมูลที่เกี่ยวข้องกับองค์กร เพื่อเตรียมระบบให้เหมาะสมกับการใช้งาน",
          "3. เริ่มใช้งานได้ทันที — ทดลองใช้ระบบ Gavalon และสัมผัสการบริหารจัดการกฎหมายที่ง่ายและเป็นระบบมากขึ้น",
        ],
      },
      {
        heading: "ทดลองใช้งาน Gavalon ฟรี 30 วัน",
        paragraphs: [
          "สัมผัสประสบการณ์การบริหารจัดการกฎหมายสำหรับองค์กร พร้อมทดลองใช้งานฟังก์ชันต่าง ๆ ของ Gavalon ได้ฟรี 30 วัน",
          "เงื่อนไขการทดลองใช้งานเป็นไปตามที่บริษัทกำหนด",
        ],
      },
      {
        heading: "สิทธิพิเศษสำหรับลูกค้า I-Law เดิม",
        paragraphs: [
          "สำหรับองค์กรที่ใช้งานระบบ I-Law อยู่แล้ว สามารถอัปเกรดมาใช้งาน Gavalon ได้โดยไม่ต้องสมัครบัญชีใหม่",
          "สามารถเข้าสู่ระบบด้วย User และ Password เดิม และเริ่มต้นใช้งาน Gavalon ได้ทันที — ลูกค้า I-Law เดิม อัปเกรดใช้ Gavalon ฟรี",
        ],
      },
      {
        heading: "Workshop Gavalon — กฎหมายใหม่เกี่ยวข้องกับธุรกิจของคุณหรือไม่?",
        paragraphs: [
          "ร่วมเรียนรู้แนวทางการใช้งาน Gavalon เพื่อช่วยองค์กรติดตามและบริหารกฎหมายที่เกี่ยวข้องกับธุรกิจ พร้อมทดลองใช้งานระบบจริงและสอบถามข้อมูลกับทีมผู้เชี่ยวชาญ",
          "การใช้งาน Gavalon 30 นาที พร้อมถาม–ตอบ — ลงทะเบียนเข้าร่วม Workshop ได้ผ่านฟอร์ม Microsoft Forms ในหน้าข่าวนี้",
        ],
        venue: "วันพุธที่ 19 สิงหาคม 2569 • เวลา 13.30 – 14.30 น. • อบรมออนไลน์ผ่าน Microsoft Teams",
        agenda: [
          {
            time: "13.30 – 13.45",
            title: "Introduction ระบบ Gavalon โดยผู้เชี่ยวชาญ NPC",
          },
          {
            time: "13.45 – 14.00",
            title: "ทดลองใช้งานระบบ Gavalon",
          },
          {
            time: "14.00 – 14.30",
            title: "ถาม – ตอบ และแลกเปลี่ยนประเด็นการใช้งาน",
          },
        ],
      },
      {
        heading: "เปลี่ยนการติดตามกฎหมายที่ซับซ้อน ให้เป็นเรื่องง่ายสำหรับองค์กร",
        paragraphs: [
          "ไม่ว่าจะเป็นการติดตามกฎหมายใหม่ การค้นหากฎหมายที่เกี่ยวข้อง การบันทึกความสอดคล้อง หรือการตรวจสอบข้อมูลย้อนหลัง Gavalon ช่วยให้องค์กรสามารถบริหารจัดการงานด้านกฎหมายได้ครบถ้วนในระบบเดียว",
          "เริ่มต้นทดลองใช้งาน Gavalon วันนี้",
        ],
      },
      {
        heading: "ติดต่อสอบถาม",
        paragraphs: [
          "เว็บไซต์: www.npc-gavalon.com",
          "โทร: 038-977700",
          "อีเมล: gavalon.support@npc-se.co.th",
        ],
      },
    ],
    websiteUrl: "https://npc-gavalon.com",
    registrationUrl:
      "https://forms.cloud.microsoft/pages/responsepage.aspx?id=4_Bt3JIWgkSqVXKeCl6DYTyOkq-KEsxBsxzIz3MjMR5URjNPVjI0MEhQTUpZMENOSDhVM0M0UDhLMS4u&origin=QRCode&qrcodeorigin=presentation&route=shorturl",
    website: {
      title: "ทดลองใช้ Gavalon ฟรี 30 วัน",
      description:
        "เริ่มต้นทดลองใช้งานระบบบริหารจัดการกฎหมายสำหรับองค์กร หรือลงทะเบียนเข้าร่วม Workshop ออนไลน์",
      button: "ลงทะเบียนทดลองใช้งาน",
    },
    media: {
      heroImage: gavalonEventHero.src,
      heroAlt: "Workshop GAVALON ระบบบริหารจัดการกฎหมายสำหรับองค์กร",
      gallery: [
        {
          title: "Workshop GAVALON — ระบบบริหารจัดการกฎหมาย",
          ratio: "wide",
          image: gavalonEventHero.src,
          alt: "โปสเตอร์ Workshop GAVALON กฎหมายใหม่เกี่ยวข้องกับธุรกิจของคุณหรือไม่",
        },
      ],
    },
  },
  {
    slug: "npc-aileen-solutions-gavalon-partnership",
    title: "NPC จับมือ Aileen Solutions ร่วมพัฒนา GAVALON ระบบบริหารจัดการกฎหมายเพื่ออนาคตที่ยั่งยืน",
    category: "Partnership",
    date: "31 กรกฎาคม 2026",
    publishedAt: "2026-07-31",
    location: "ประเทศไทย",
    summary:
      "บริษัท NPC Safety and Environmental Service จำกัด ร่วมกับ Aileen Solutions พัฒนา GAVALON ระบบบริหารจัดการกฎหมายและข้อกำหนดที่เกี่ยวข้อง เพื่อช่วยให้องค์กรติดตาม ตรวจสอบ และบริหารการปฏิบัติตามกฎหมายได้อย่างเป็นระบบ",
    excerpt:
      "ความร่วมมือครั้งนี้ผสานความเชี่ยวชาญของ NPC ในด้านความปลอดภัย อาชีวอนามัย และสิ่งแวดล้อม เข้ากับศักยภาพด้านเทคโนโลยีของ Aileen Solutions เพื่อยกระดับการบริหารจัดการกฎหมายและข้อกำหนดอย่างมีประสิทธิภาพ",
    tags: [
      "GAVALON",
      "NPC",
      "Legal Compliance",
      "Sustainability",
      "Partnership",
      "Governance",
    ],
    highlights: [
      "NPC และ Aileen Solutions ร่วมพัฒนา GAVALON ระบบบริหารจัดการกฎหมาย",
      "ผสานความเชี่ยวชาญด้าน HSE กับเทคโนโลยีและการพัฒนาระบบ",
      "สนับสนุนการกำกับดูแลกิจการและการดำเนินธุรกิจอย่างยั่งยืน",
    ],
    sections: [
      {
        heading: "ความร่วมมือเพื่อยกระดับการบริหารจัดการกฎหมาย",
        paragraphs: [
          "บริษัท NPC Safety and Environmental Service จำกัด ร่วมกับบริษัท Aileen Solutions เดินหน้าพัฒนา GAVALON ระบบบริหารจัดการกฎหมายและข้อกำหนดที่เกี่ยวข้อง เพื่อช่วยให้องค์กรสามารถติดตาม ตรวจสอบ และบริหารการปฏิบัติตามกฎหมายได้อย่างเป็นระบบ",
          "ความร่วมมือครั้งนี้ผสานความเชี่ยวชาญของ NPC ในด้านความปลอดภัย อาชีวอนามัย และสิ่งแวดล้อม เข้ากับศักยภาพด้านเทคโนโลยีและการพัฒนาระบบของ Aileen Solutions เพื่อยกระดับกระบวนการทำงาน ลดความซับซ้อนในการจัดการข้อมูล และสนับสนุนให้องค์กรเตรียมพร้อมต่อการเปลี่ยนแปลงของข้อกฎหมายได้อย่างมีประสิทธิภาพ",
        ],
      },
      {
        heading: "GAVALON — มากกว่าเครื่องมือจัดเก็บข้อมูล",
        paragraphs: [
          "GAVALON ไม่ได้เป็นเพียงเครื่องมือสำหรับจัดเก็บข้อมูลด้านกฎหมาย แต่ยังเป็นส่วนสำคัญในการส่งเสริมการกำกับดูแลกิจการที่ดี ลดความเสี่ยงจากการไม่ปฏิบัติตามข้อกำหนด และสนับสนุนการดำเนินธุรกิจอย่างรับผิดชอบและยั่งยืน",
        ],
      },
      {
        heading: "มุ่งสู่อนาคตที่มีมาตรฐานและยั่งยืน",
        paragraphs: [
          "ความร่วมมือระหว่าง NPC และ Aileen Solutions สะท้อนถึงความมุ่งมั่นในการนำความรู้ ความเชี่ยวชาญ และเทคโนโลยีมาพัฒนาโซลูชันที่ตอบโจทย์องค์กรยุคใหม่ พร้อมขับเคลื่อนภาคธุรกิจไปสู่อนาคตที่มีมาตรฐาน ปลอดภัย และยั่งยืนมากยิ่งขึ้น",
        ],
      },
    ],
    websiteUrl: "https://npc-gavalon.com",
    website: {
      title: "เว็บไซต์ GAVALON",
      description:
        "ไม่พลาดทุกกฎหมายสำคัญ ตรวจทานโดยทีมผู้เชี่ยวชาญด้านกฎหมาย — เรียนรู้เพิ่มเติมเกี่ยวกับแพลตฟอร์มบริหารจัดการกฎหมายและข้อกำหนด",
      button: "เยี่ยมชม npc-gavalon.com",
    },
    media: {
      heroImage: gavalonNpcHero.src,
      heroAlt: "NPC และ Aileen Solutions ร่วมพัฒนา GAVALON ระบบบริหารจัดการกฎหมาย",
      gallery: [
        {
          title: "GAVALON — ระบบบริหารจัดการกฎหมายและข้อกำหนด",
          ratio: "wide",
          image: gavalonNpcHero.src,
          alt: "ภาพประกอบความร่วมมือ NPC และ Aileen Solutions ในการพัฒนา GAVALON",
        },
      ],
    },
  },
  {
    slug: "enterprise-process-visibility-sustainable-operations-2026",
    hidden: true,
    title:
      "Aileen Solutions ร่วมกับ Nintex Thailand จัดสัมมนา Enterprise Process Visibility ยกระดับกระบวนการองค์กรสู่ความยั่งยืน",
    category: "Seminar",
    date: "9 กรกฎาคม 2026",
    publishedAt: "2026-07-10",
    location: "Hotel Nikko Amata City, จังหวัดชลบุรี",
    summary:
      "Aileen Solutions ร่วมกับ Nintex Thailand จัดงานสัมมนา Enterprise Process Visibility เพราะกระบวนการที่ดี คือรากฐานของความยั่งยืน โดยมีผู้บริหารและตัวแทนจากองค์กรชั้นนำเข้าร่วมแลกเปลี่ยนมุมมองด้านการพัฒนากระบวนการ การประยุกต์ใช้ AI และ Automation",
    excerpt:
      "งานสัมมนาครั้งนี้จัดขึ้นภายใต้แนวคิดว่า ความสำเร็จของ Digital Transformation ไม่ได้เริ่มต้นจากเทคโนโลยีเพียงอย่างเดียว แต่ต้องเริ่มจากการมองเห็นและเข้าใจกระบวนการทำงานขององค์กรอย่างชัดเจน",
    tags: [
      "Enterprise Process Visibility",
      "Nintex Thailand",
      "Digital Transformation",
      "AI",
      "Automation",
      "Kaizen",
      "Net Zero",
      "Sustainability",
    ],
    highlights: [
      "ผู้บริหารและตัวแทนองค์กรชั้นนำร่วมแลกเปลี่ยน Process, AI และ Automation",
      "กรอบ SEE – IMPROVE – SUSTAIN สู่การดำเนินงานยั่งยืน",
      "บรรยายจาก Aileen Solutions, Nintex Thailand และกรณีศึกษาจาก Global Green Chemicals",
    ],
    sections: [
      {
        heading: "งานสัมมนา Enterprise Process Visibility",
        paragraphs: [
          "Aileen Solutions ร่วมกับ Nintex Thailand จัดงานสัมมนา Enterprise Process Visibility เพราะกระบวนการที่ดี คือรากฐานของความยั่งยืน เมื่อวันที่ 9 กรกฎาคม 2026 ณ Hotel Nikko Amata City จังหวัดชลบุรี โดยมีผู้บริหารและตัวแทนจากองค์กรชั้นนำเข้าร่วมแลกเปลี่ยนมุมมองด้านการพัฒนากระบวนการ การประยุกต์ใช้ AI และ Automation ตลอดจนแนวทางขับเคลื่อนองค์กรสู่เป้าหมายด้านความยั่งยืน",
          "งานสัมมนาครั้งนี้จัดขึ้นภายใต้แนวคิดว่า ความสำเร็จของ Digital Transformation ไม่ได้เริ่มต้นจากเทคโนโลยีเพียงอย่างเดียว แต่ต้องเริ่มจากการมองเห็นและเข้าใจกระบวนการทำงานขององค์กรอย่างชัดเจน หรือ Enterprise Process Visibility เพื่อให้องค์กรสามารถตรวจสอบความซ้ำซ้อน ลดขั้นตอนที่ไม่สร้างคุณค่า และวางมาตรฐานการทำงานที่พร้อมต่อยอดด้วยเทคโนโลยีได้อย่างมีประสิทธิภาพ",
        ],
      },
      {
        heading: "Process Clarity to Sustainable Operations: SEE – IMPROVE – SUSTAIN",
        paragraphs: [
          "ภายในงาน ผู้เข้าร่วมได้เรียนรู้แนวคิด Process Clarity to Sustainable Operations ผ่านกรอบการดำเนินงาน 3 ขั้นตอน ได้แก่ SEE – IMPROVE – SUSTAIN ตั้งแต่การมองเห็นกระบวนการในภาพรวม การปรับปรุงกระบวนการด้วยแนวคิด ECRS และ Kaizen ไปจนถึงการรักษามาตรฐานการดำเนินงาน เพื่อสนับสนุนเป้าหมาย Net Zero และการเติบโตอย่างยั่งยืนในระยะยาว",
        ],
      },
      {
        heading: "Digitizing the DNA of Sustainability: Process First, Automation Next, Sustainability Always",
        paragraphs: [
          "คุณปราโมทย์ ตรีนาคสิทธิ์ จาก Aileen Solutions ถ่ายทอดแนวคิดในหัวข้อ Digitizing the DNA of Sustainability: Process First, Automation Next, Sustainability Always โดยเน้นย้ำถึงความสำคัญของการวางรากฐานกระบวนการให้ชัดเจน ก่อนนำ Automation และเทคโนโลยีดิจิทัลเข้ามาสนับสนุนการดำเนินงาน",
        ],
      },
      {
        heading: "Ultimate Power + Ultimate Control: Redefining Operations in the Age of AI",
        paragraphs: [
          "ด้านทีมงานจาก Nintex Thailand นำโดยคุณเอก งามเจริญ และคุณพุทธพงศ์ เชี่ยวเวช ร่วมบรรยายในหัวข้อ Ultimate Power + Ultimate Control: Redefining Operations in the Age of AI พร้อมนำเสนอแนวทางการใช้เครื่องมือด้าน Process Management และ Automation เพื่อเพิ่มประสิทธิภาพ สร้างความโปร่งใส และช่วยให้องค์กรสามารถบริหารจัดการกระบวนการได้อย่างเป็นระบบในยุค AI",
        ],
      },
      {
        heading: "Success Story: From Experience to Excellence",
        paragraphs: [
          "อีกหนึ่งช่วงสำคัญของงานคือการแบ่งปันประสบการณ์จริงในหัวข้อ Success Story: From Experience to Excellence โดยคุณพร้อมพร อิศรางกูร ณ อยุธยา Vice President – Operations จาก Global Green Chemicals Public Company Limited ซึ่งถ่ายทอดแนวทางการพัฒนากระบวนการจากประสบการณ์หน้างาน และการยกระดับการดำเนินงานให้เกิดผลลัพธ์ที่สามารถนำไปใช้ได้จริงในภาคอุตสาหกรรม",
        ],
      },
      {
        heading: "ขับเคลื่อนสู่การดำเนินงานที่ยั่งยืน",
        paragraphs: [
          "งานสัมมนาครั้งนี้สะท้อนให้เห็นว่า การสร้างความยั่งยืนในองค์กรต้องเริ่มจากกระบวนการที่ชัดเจน มีมาตรฐาน และสามารถวัดผลได้ เมื่อองค์กรเข้าใจกระบวนการของตนเองอย่างรอบด้าน การนำ AI, Automation และเทคโนโลยีดิจิทัลเข้ามาประยุกต์ใช้จึงสามารถสร้างคุณค่า ลดต้นทุน และสนับสนุนการเติบโตได้อย่างเต็มศักยภาพ",
          "Aileen Solutions และ Nintex Thailand ขอขอบคุณผู้บริหาร วิทยากร และผู้เข้าร่วมงานทุกท่านที่ร่วมแลกเปลี่ยนความรู้และประสบการณ์ พร้อมเป็นส่วนหนึ่งของการยกระดับกระบวนการองค์กรจาก Process Visibility ไปสู่การดำเนินงานที่มีประสิทธิภาพและยั่งยืนอย่างเป็นรูปธรรม",
        ],
      },
    ],
    media: {
      heroImage: epvHero.src,
      heroAlt: "งานสัมมนา Enterprise Process Visibility โดย Aileen Solutions และ Nintex Thailand",
      gallery: [
        {
          title: "การบรรยายและแลกเปลี่ยนความรู้ภายในงาน",
          ratio: "wide",
          image: epvHero.src,
          alt: "วิทยากรและผู้เข้าร่วมงานในงานสัมมนา Enterprise Process Visibility",
        },
        {
          title: "การนำเสนอแนวทาง Process Management และ Automation",
          ratio: "wide",
          image: epvGallery2.src,
          alt: "การบรรยายจากทีมงานในงานสัมมนา Enterprise Process Visibility",
        },
        {
          title: "ผู้เข้าร่วมงานจากองค์กรชั้นนำ",
          ratio: "wide",
          image: epvGallery3.src,
          alt: "ผู้บริหารและตัวแทนองค์กรเข้าร่วมงานสัมมนา",
        },
        {
          title: "บรรยากาศการแลกเปลี่ยนมุมมองด้านความยั่งยืน",
          ratio: "wide",
          image: epvGallery4.src,
          alt: "บรรยากาศการแลกเปลี่ยนความรู้ในงานสัมมนา",
        },
        {
          title: "ภาพรวมงานสัมมนา Enterprise Process Visibility",
          ratio: "wide",
          image: epvAlbum.src,
          alt: "ภาพรวมงานสัมมนา Enterprise Process Visibility ณ Hotel Nikko Amata City",
        },
      ],
    },
  },
  {
    slug: "aileen-solutions-at-tnche-asia-2026",
    title: "พบกับ Aileen Solutions ได้ในงาน TNChE Asia 2026",
    category: "Conference",
    date: "9-11 มิถุนายน 2026",
    publishedAt: "2026-06-09",
    location: "Dusit Thani Pattaya, Thailand • Booth A8",
    summary:
      "Aileen Solutions เข้าร่วมงาน TNChE Asia 2026 เพื่อนำเสนอแนวคิดด้าน AI, Automation, Safety และ Digital Solutions ภายใต้ธีม “INSPIRING Innovations and Digital Solutions for Asia Sustainability” ที่มุ่งขับเคลื่อนอุตสาหกรรมสู่อนาคตที่แข็งแกร่ง ยั่งยืน และมีคุณภาพมากขึ้น",
    excerpt:
      "พบกับทีม Aileen Solutions ระหว่างวันที่ 9-11 มิถุนายน 2026 ณ Dusit Thani Pattaya, Thailand บูธ A8 เพื่อร่วมแลกเปลี่ยนมุมมองด้านนวัตกรรม เทคโนโลยีดิจิทัล และโซลูชันที่ตอบโจทย์การพัฒนาอุตสาหกรรมอย่างยั่งยืนในระดับเอเชีย",
    tags: [
      "TNChE Asia 2026",
      "ROPTC 2026",
      "AI",
      "Automation",
      "Safety",
      "Digital Transformation",
      "Sustainability",
    ],
    highlights: [
      "พบกับ Aileen Solutions ที่บูธ A8 ในงาน TNChE Asia 2026",
      "นำเสนอแนวคิดด้าน AI, Automation และ Digital Solutions เพื่อความยั่งยืน",
      "สะท้อนบทบาทของนวัตกรรมในการขับเคลื่อนอุตสาหกรรมเอเชียสู่อนาคต",
    ],
    sections: [
      {
        heading: "Aileen Solutions ในเวที TNChE Asia 2026",
        paragraphs: [
          "Aileen Solutions พร้อมเข้าร่วมงาน TNChE Asia 2026 ระหว่างวันที่ 9-11 มิถุนายน 2026 ณ Dusit Thani Pattaya, Thailand โดยเราจะอยู่ที่ Booth A8 เพื่อพบปะผู้เข้าร่วมงานจากภาคอุตสาหกรรม นักวิชาการ และผู้เชี่ยวชาญในสายวิศวกรรมเคมีและเคมีประยุกต์จากหลากหลายประเทศในเอเชีย",
          "การเข้าร่วมงานครั้งนี้เป็นอีกก้าวสำคัญของ Aileen Solutions ในการนำเสนอแนวทางการประยุกต์ใช้เทคโนโลยีดิจิทัลและโซลูชันอัจฉริยะ เพื่อยกระดับประสิทธิภาพ ความปลอดภัย และความสามารถในการแข่งขันขององค์กรอุตสาหกรรมอย่างเป็นรูปธรรม",
        ],
      },
      {
        heading: "ธีมงานที่สะท้อนอนาคตของอุตสาหกรรม",
        paragraphs: [
          "ภายใต้ธีม “INSPIRING Innovations and Digital Solutions for Asia Sustainability” งาน TNChE Asia 2026 สะท้อนให้เห็นถึงบทบาทของนวัตกรรม เทคโนโลยีดิจิทัล และแนวคิดด้านความยั่งยืน ที่กำลังกลายเป็นหัวใจสำคัญของการพัฒนาอุตสาหกรรมในภูมิภาคเอเชีย",
          "Aileen Solutions มองว่าแนวคิดเรื่อง Sustainability ไม่ได้จำกัดอยู่เพียงการลดผลกระทบต่อสิ่งแวดล้อมเท่านั้น แต่ยังรวมถึงการออกแบบกระบวนการทำงานที่โปร่งใส ตรวจสอบได้ ปลอดภัย และรองรับการเติบโตของธุรกิจในระยะยาว",
        ],
      },
      {
        heading: "สิ่งที่คุณจะได้พบที่บูธ A8",
        paragraphs: [
          "ภายในบูธของ Aileen Solutions ผู้เข้าร่วมงานจะได้พบกับมุมมองและตัวอย่างการใช้งานจริงในด้าน AI, Automation, Safety และ Digital Transformation ที่สามารถนำไปประยุกต์ใช้กับบริบทของโรงงานและองค์กรอุตสาหกรรมได้จริง",
          "เราพร้อมแลกเปลี่ยนแนวคิดเกี่ยวกับการมองเห็นกระบวนการทำงานทั้งระบบ การเพิ่มประสิทธิภาพการดำเนินงาน การลดความเสี่ยง และการยกระดับการตัดสินใจด้วยข้อมูล เพื่อสร้างองค์กรที่แข็งแกร่ง มีคุณภาพ และพร้อมสำหรับอนาคต",
        ],
      },
    ],
    media: {
      heroImage: tncheHero.src,
      heroAlt: "ทีม Aileen Solutions ที่บูธในงาน TNChE Asia 2026",
      gallery: [
        {
          title: "บรรยากาศการพูดคุยและแนะนำโซลูชันภายในบูธ",
          ratio: "wide",
          image: tncheGalleryBooth.src,
          alt: "ทีม Aileen Solutions แนะนำโซลูชันให้ผู้เข้าร่วมงาน TNChE Asia 2026",
        },
        {
          title: "การสาธิตการใช้งานและอธิบายกระบวนการทำงาน",
          ratio: "wide",
          image: tncheGalleryDemo.src,
          alt: "การสาธิตระบบและอธิบายโซลูชันภายในงาน TNChE Asia 2026",
        },
        {
          title: "ทีมงาน Aileen Solutions ณ Booth A8",
          ratio: "wide",
          image: tncheHero.src,
          alt: "ทีม Aileen Solutions ถ่ายภาพร่วมกันที่บูธ A8 ในงาน TNChE Asia 2026",
        },
      ],
    },
  },
  {
    slug: "safety-together-sustainable-safety-culture",
    title: "Safety Together: สร้างวัฒนธรรมความปลอดภัยอย่างยั่งยืน",
    category: "Event Highlight",
    date: "12 พฤษภาคม 2026",
    publishedAt: "2026-05-12",
    location: "การไฟฟ้าฝ่ายผลิตแห่งประเทศไทย (EGAT)",
    summary:
      "Aileen Solutions เข้าร่วมงาน Safety Together ของ EGAT เพื่อแลกเปลี่ยนแนวทางยกระดับความปลอดภัยในองค์กร ผ่านการผสาน AI, เทคโนโลยีดิจิทัล, กระบวนการทำงาน และความร่วมมือภายในองค์กรอย่างเป็นระบบ",
    excerpt:
      "งานนี้มุ่งเน้นการสร้างความตระหนักด้านความปลอดภัย และพัฒนา Safety Culture ที่ยั่งยืนด้วยแนวคิดการทำงานร่วมกันระหว่างคน กระบวนการ และเทคโนโลยี",
    tags: ["Safety Culture", "EGAT", "AI", "Digital Technology", "Process"],
    highlights: [
      "ส่งเสริมความตระหนักด้านความปลอดภัยในองค์กร",
      "แลกเปลี่ยนประสบการณ์และแนวปฏิบัติจากหลายอุตสาหกรรม",
      "เน้นบทบาทของ AI, Digital, Process และ Collaboration",
    ],
    sections: [
      {
        heading: "ภาพรวมของงาน",
        paragraphs: [
          "บริษัท ไอลีน โซลูชั่น จำกัด ได้มีโอกาสเข้าร่วมงาน “Safety Together” ซึ่งจัดโดย การไฟฟ้าฝ่ายผลิตแห่งประเทศไทย (EGAT) โดยมีวัตถุประสงค์เพื่อส่งเสริมความตระหนักด้านความปลอดภัย และสร้างวัฒนธรรมความปลอดภัยในองค์กรอย่างยั่งยืน",
          "ภายในงานมีการแลกเปลี่ยนความรู้ ประสบการณ์ และแนวปฏิบัติที่ดีจากผู้เชี่ยวชาญในหลากหลายอุตสาหกรรม เพื่อยกระดับมาตรฐานความปลอดภัยให้สอดคล้องกับความท้าทายของการดำเนินงานในปัจจุบัน",
        ],
      },
      {
        heading: "มุมมองจาก Aileen Solutions",
        paragraphs: [
          "Aileen Solutions ให้ความสำคัญกับการนำ AI, เทคโนโลยีดิจิทัล และกระบวนการทำงานที่ออกแบบอย่างเหมาะสม มาใช้เป็นเครื่องมือขับเคลื่อนความปลอดภัยในองค์กรอย่างเป็นระบบ ไม่ใช่เพียงการแก้ปัญหาเฉพาะหน้า",
          "การสร้างความปลอดภัยที่ยั่งยืนจำเป็นต้องอาศัยการทำงานร่วมกันของคนในองค์กร ข้อมูลที่ถูกต้อง และกระบวนการที่ตรวจสอบได้ เพื่อให้ทุกฝ่ายสามารถตัดสินใจและตอบสนองได้อย่างรวดเร็วและแม่นยำ",
        ],
      },
    ],
    media: {
      heroImage: egatHero.src,
      heroAlt: "ทีมงาน Aileen Solutions ภายในงาน EGAT Safety Together 2025",
      gallery: [
        {
          title: "บูธ Aileen Solutions ภายในงาน",
          ratio: "portrait",
          image: egatGalleryBooth.src,
          alt: "บูธ Aileen Solutions ในงาน EGAT Safety Together 2025",
        },
        {
          title: "ทีมงานร่วมต้อนรับผู้เข้าร่วมงาน",
          ratio: "portrait",
          image: egatGalleryTeam.src,
          alt: "ทีมงาน Aileen Solutions หน้าบูธในงาน EGAT Safety Together 2025",
        },
        {
          title: "ภาพรวมทีมงานและบรรยากาศงาน",
          ratio: "wide",
          image: egatHero.src,
          alt: "ภาพรวมทีมงาน Aileen Solutions ในงาน EGAT Safety Together 2025",
        },
      ],
    },
  },
  {
    slug: "ai-driven-industrial-transformation-sistam-2025",
    title: "AI-Driven Industrial Transformation at SISTAM 2025",
    category: "Conference",
    date: "11 พฤษภาคม 2026",
    publishedAt: "2026-05-11",
    location: "SISTAM 2025",
    summary:
      "Aileen Solutions เข้าร่วมงาน SISTAM 2025 เพื่อนำเสนอแนวคิด AI-Driven Industrial Operations ผ่านการผสาน Process, Automation และ AI เพื่อช่วยให้องค์กรบริหารจัดการได้อย่างมีประสิทธิภาพ ปลอดภัย และยั่งยืน",
    excerpt:
      "เวทีนี้สะท้อนทิศทางของอุตสาหกรรมที่กำลังเปลี่ยนผ่านจากการปรับปรุงเฉพาะจุด ไปสู่การยกระดับการดำเนินงานทั้งระบบด้วยข้อมูล กระบวนการ และ AI",
    tags: ["SISTAM 2025", "Industrial Transformation", "Automation", "AI", "Operations"],
    highlights: [
      "นำเสนอแนวคิด AI-Driven Industrial Operations",
      "เชื่อม Process, Automation และ AI เข้าด้วยกัน",
      "มุ่งสู่องค์กรที่มีประสิทธิภาพ ปลอดภัย และยั่งยืน",
    ],
    sections: [
      {
        heading: "เวทีเทคโนโลยีอุตสาหกรรมสมัยใหม่",
        paragraphs: [
          "บริษัท ไอลีน โซลูชั่น จำกัด ได้เข้าร่วมงาน SISTAM 2025 (Smart Industrial Safety Technology & Advanced Management) ซึ่งเป็นเวทีสำคัญในการนำเสนอเทคโนโลยีและนวัตกรรมด้านอุตสาหกรรม ความปลอดภัย และการบริหารจัดการสมัยใหม่",
          "บรรยากาศของงานสะท้อนให้เห็นถึงแนวโน้มของภาคอุตสาหกรรมที่ให้ความสำคัญกับทั้งประสิทธิภาพ ความปลอดภัย และความสามารถในการปรับตัวอย่างรวดเร็วในโลกการทำงานที่ซับซ้อนมากขึ้น",
        ],
      },
      {
        heading: "แนวคิดที่ Aileen Solutions นำเสนอ",
        paragraphs: [
          "ภายในงาน Aileen Solutions ได้นำเสนอแนวคิดการยกระดับองค์กรสู่ AI-Driven Industrial Operations ผ่านการผสาน Process, Automation และ AI เพื่อช่วยให้องค์กรสามารถบริหารจัดการได้อย่างมีประสิทธิภาพ ปลอดภัย และยั่งยืน",
          "แนวทางดังกล่าวช่วยให้องค์กรมองเห็นภาพการทำงานแบบ end-to-end ลดจุดคอขวด เพิ่มความสามารถในการติดตามสถานะ และต่อยอดสู่การตัดสินใจที่ขับเคลื่อนด้วยข้อมูลและ AI ได้อย่างเป็นรูปธรรม",
        ],
      },
    ],
    media: {
      heroImage: sistamHero.src,
      heroAlt: "ทีมงาน Aileen Solutions ภายในงาน SISTAM 2025",
      gallery: [
        {
          title: "พื้นที่บูธและการจัดแสดงโซลูชัน",
          ratio: "portrait",
          image: sistamGalleryBooth.src,
          alt: "พื้นที่บูธของ Aileen Solutions ภายในงาน SISTAM 2025",
        },
        {
          title: "การพูดคุยกับผู้เข้าร่วมงาน",
          ratio: "wide",
          image: sistamGalleryVisitor.src,
          alt: "การแนะนำโซลูชันแก่ผู้เข้าร่วมงาน SISTAM 2025",
        },
        {
          title: "ทีมงานและภาพบรรยากาศหน้า backdrop",
          ratio: "wide",
          image: sistamGalleryBackdrop.src,
          alt: "ทีมงาน Aileen Solutions ถ่ายภาพหน้า backdrop SISTAM 2025",
        },
      ],
    },
  },
  {
    slug: "ai-driven-automation-rayong-industrial-seminar",
    title: "AI-Driven Automation: ขับเคลื่อนองค์กรอุตสาหกรรมสู่ยุคอัจฉริยะ",
    category: "Seminar",
    date: "10 พฤษภาคม 2026",
    publishedAt: "2026-05-10",
    location: "จังหวัดระยอง",
    summary:
      "Aileen Solutions จัดสัมมนา AI-Driven Automation ที่จังหวัดระยอง เพื่อแบ่งปันแนวทางการประยุกต์ใช้ AI และ Automation ในภาคอุตสาหกรรม พร้อมชูแนวคิดการสร้าง AI-Driven Organization ที่เพิ่มประสิทธิภาพ ลดต้นทุน และสร้างความได้เปรียบในการแข่งขัน",
    excerpt:
      "สัมมนาครั้งนี้เน้นการเชื่อมโยง Process, Automation และ AI เข้ากับบริบทการทำงานจริงขององค์กรอุตสาหกรรม เพื่อให้การเปลี่ยนผ่านสู่ยุคอัจฉริยะเกิดขึ้นได้จริง",
    tags: ["Rayong", "Seminar", "AI", "Automation", "Industrial Organization"],
    highlights: [
      "รวมผู้บริหารและผู้เชี่ยวชาญจากหลายองค์กร",
      "แชร์ประสบการณ์การใช้ AI และ Automation ในภาคอุตสาหกรรม",
      "มุ่งสู่ AI-Driven Organization อย่างยั่งยืน",
    ],
    sections: [
      {
        heading: "เป้าหมายของการสัมมนา",
        paragraphs: [
          "บริษัท ไอลีน โซลูชั่น จำกัด ได้จัดงานสัมมนา “AI-Driven Automation” ณ จังหวัดระยอง เพื่อแบ่งปันแนวทางและประสบการณ์ในการนำเทคโนโลยี AI และ Automation มาประยุกต์ใช้ในภาคอุตสาหกรรม โดยมีผู้บริหารและผู้เชี่ยวชาญจากหลากหลายองค์กรเข้าร่วม",
          "หัวใจสำคัญของงานคือการช่วยให้องค์กรเห็นภาพการเปลี่ยนผ่านจากการทำงานแบบเดิม ไปสู่การดำเนินงานที่ขับเคลื่อนด้วยข้อมูล ระบบอัตโนมัติ และ AI ที่ตอบโจทย์การทำงานจริง",
        ],
      },
      {
        heading: "แนวทางการขับเคลื่อนองค์กร",
        paragraphs: [
          "งานสัมมนาครั้งนี้มุ่งเน้นการยกระดับองค์กรสู่การเป็น AI-Driven Organization ผ่านการผสาน Process, Automation และ AI เพื่อเพิ่มประสิทธิภาพ ลดต้นทุน และสร้างความได้เปรียบในการแข่งขันอย่างยั่งยืน",
          "Aileen Solutions เชื่อว่าการเปลี่ยนแปลงที่มีผลลัพธ์จริง ต้องเริ่มจากการทำความเข้าใจกระบวนการธุรกิจ แล้วจึงค่อยเลือกใช้ Automation และ AI ให้เหมาะสมกับเป้าหมายขององค์กร",
        ],
      },
    ],
    media: {
      heroImage: aiDrivenHero.src,
      heroAlt: "บรรยากาศการบรรยายในงานสัมมนา AI-Driven Automation",
      gallery: [
        {
          title: "จุดลงทะเบียนและพื้นที่ประชาสัมพันธ์ภายในงาน",
          ratio: "wide",
          image: aiDrivenGalleryBooth.src,
          alt: "จุดลงทะเบียนและบูธประชาสัมพันธ์ในงาน AI-Driven Automation",
        },
        {
          title: "บรรยากาศ session presentation ภายในสัมมนา",
          ratio: "wide",
          image: aiDrivenHero.src,
          alt: "วิทยากรกำลังนำเสนอภายในงานสัมมนา AI-Driven Automation",
        },
      ],
    },
  },
];

export function getVisibleNewsArticles() {
  return NEWS_ARTICLES.filter((article) => !article.hidden);
}

export function getNewsArticle(slug) {
  const article = NEWS_ARTICLES.find((item) => item.slug === slug);
  if (!article || article.hidden) return undefined;
  return article;
}
